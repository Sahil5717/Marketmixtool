...
const reOpt=async()=>{if(!D)return;
  if(apiMode){
    const mt=hillMode?"hill":"power_law";
...
<div style={{display:"flex",gap:4}}>{["linear","last_touch","position_based"].map(m=><Btn key={m} primary={atM===m} onClick={()=>setAtM(m)} style={{fontSize:10,padding:"5px 12px"}}>{m.replace(/_/g," ")}</Btn>)}</div></div>
<table><thead><tr><th>Channel</th><th>Type</th><th style={{textAlign:"right"}}>Last Touch</th><th style={{textAlign:"right"}}>Linear</th><th style={{textAlign:"right"}}>Position-Based</th></tr></thead>
<tbody>{Object.keys(CH).map(ch=>{const lt=D.attr.last_touch[ch]||0,ln=D.attr.linear[ch]||0,pb=D.attr.position_based[ch]||0;return<tr key={ch}><td><span style={{color:CH[ch]?.color,fontWeight:600}}>{FN(ch)}</span></td><td><Badge color={CH[ch]?.type==="online"?V.blue:V.violet}>{CH[ch]?.type}</Badge></td><td style={{textAlign:"right",fontWeight:atM==="last_touch"?700:400,color:atM==="last_touch"?V.teal:V.text}}>{F(lt)}</td><td style={{textAlign:"right",fontWeight:atM==="linear"?700:400,color:atM==="linear"?V.teal:V.text}}>{F(ln)}</td><td style={{textAlign:"right",fontWeight:atM==="position_based"?700:400,color:atM==="position_based"?V.teal:V.text}}>{F(pb)}</td></tr>})}</tbody></table></Card>
...
<div style={{marginBottom:16}}><label style={{fontSize:11,color:V.textMuted,fontWeight:600,display:"block",marginBottom:6}}>Response Curve Model</label><div style={{display:"flex",gap:4}}><Btn primary={!hillMode} onClick={()=>setHillMode(false)} style={{fontSize:10,padding:"4px 10px"}}>Power-Law</Btn><Btn primary={hillMode} onClick={()=>setHillMode(true)} style={{fontSize:10,padding:"4px 10px"}}>Hill</Btn></div><div style={{fontSize:10,color:V.textLight,marginTop:4}}>{hillMode?"y = a·x^b / (K^b + x^b)":"y = a·x^b (log-linear)"}</div></div>
...
<Card style={{marginTop:14}}><SectionLabel>Budget Sensitivity</SectionLabel><p style={{fontSize:11,color:V.textMuted,marginBottom:10}}>Projected revenue at different budget levels using {hillMode?"Hill":"power-law"} response curves</p>
...
<tbody>{[-30,-20,-10,0,10,20,30,50].map(pct=>{const b=D.tS*(1+pct/100);const predRev=D.opt.channels.reduce((sum,c)=>{const ch=c.channel;const cv=D.curves?.[ch];if(!cv)return sum+c.oR*(1+pct/100);const s=(c.oS||c.cS)*(1+pct/100);const K=cv.avgSpend||1;const r=hillMode?((cv.a||1)*10*Math.pow(Math.max(s/12,1),cv.b||.5))/(Math.pow(K,cv.b||.5)+Math.pow(Math.max(s/12,1),cv.b||.5))*12:(cv.a||1)*Math.pow(Math.max(s/12,1),cv.b||.5)*12;return sum+r},0);const roi=(predRev-b)/Math.max(b,1);return<tr key={pct} style={{background:pct===0?V.cardBorder+"33":"transparent",fontWeight:pct===0?700:400}}><td style={{color:pct>0?V.green:pct<0?V.red:V.text}}>{pct>0?"+":""}{pct}%</td><td style={{textAlign:"right"}}>{F(b)}</td><td style={{textAlign:"right"}}>{F(predRev)}</td><td style={{textAlign:"right"}}>{FX(roi)}</td><td style={{textAlign:"right",color:pct===0?V.textMuted:roi>(D.opt.summary.oROI||0)?V.green:V.red}}>{pct===0?"Base":roi>(D.opt.summary.oROI||0)?"↑ Efficient":"↓ Diminishing"}</td></tr>})}</tbody></table></Card>

{/* Marginal ROI Table */}
{D.curves&&Object.keys(D.curves).length>0&&<Card style={{marginTop:14}}><SectionLabel>Marginal ROI at Spend Levels</SectionLabel><p style={{fontSize:11,color:V.textMuted,marginBottom:10}}>Shows diminishing returns as spend increases per channel ({hillMode?"Hill":"power-law"} model)</p>
...
<div style={{display:"flex",gap:10}}><Btn onClick={exportCSV}><Download size={13}/>CSV Plan</Btn><Btn primary><FileText size={13}/>Executive Summary</Btn></div>
</div></Card>
