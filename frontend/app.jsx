...
const reOpt=async()=>{if(!D)return;
  if(apiMode){
    const res=await apiCall(`/optimize?total_budget=${D.tS*bM}&objective=${obj}`,{method:"POST"});
    if(res){const st=await apiCall("/full-state");if(st){setD(d=>({...d,opt:st.opt,pl:st.pl}))}}
  } else {
...
<Card><SectionLabel>Allocation Detail</SectionLabel><table><thead><tr><th>Channel</th><th style={{textAlign:"right"}}>Current</th><th style={{textAlign:"right"}}>Optimized</th><th style={{textAlign:"right"}}>Change</th><th style={{textAlign:"right"}}>Proj Revenue</th><th style={{textAlign:"right"}}>Marginal ROI</th></tr></thead><tbody>{D.opt.channels.sort((a,b)=>b.chg-a.chg).map(c=><tr key={c.channel} style={{opacity:c.locked?.5:1}}><td><span style={{color:CH[c.channel]?.color,fontWeight:600}}>{FN(c.channel)}</span>{c.locked&&<Lock size={10} color={V.teal} style={{marginLeft:4}}/>}</td><td style={{textAlign:"right"}}>{F(c.cS)}</td><td style={{textAlign:"right",fontWeight:600}}>{F(c.oS)}</td><td style={{textAlign:"right",color:c.chg>0?V.green:V.red,fontWeight:700}}>{FP(c.chg)}</td><td style={{textAlign:"right"}}>{F(c.oR)}</td><td style={{textAlign:"right",color:V.teal,fontWeight:600}}>{FX(c.mROI)}</td></tr>)}</tbody></table></Card>
</div></div></div>}
